...
struct TabBar: View {
    @State var color: Color = .appColor
    @State var selectedX: CGFloat = 0
    @State var xValue: [CGFloat] = [0, 0, 0, 0, 0]

    @EnvironmentObject var navigationModel: NavigationModel
    @SceneStorage("selectedView") var selectedTab: Tab = .albums
    ...
    var body: some View {
        GeometryReader { proxy in
            ...
            .ignoresSafeArea()
            .offset(y: navigationModel.showTab ? 0 : 200)
            .accessibility(hidden: !navigationModel.showTab)
        }
    }
